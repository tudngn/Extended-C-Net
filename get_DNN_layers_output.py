...
def get_out_layers(DNNModel,data,hidden_nodes,num_output):
...
if __name__ == '__main__':
    ...
    listdir_TrainingData = os.listdir("./NumericalData/ecoli/Training/")
    ...
    # start ID for chossing DNN model
    startModelID = 0

    # Main loop to execute data operation and get the output at each layer of the DNN
    for i in range(0,len(listdir_TrainingData)):
        # Load data
        trainingData = np.load("./NumericalData/ecoli/Data/Training/" + listdir_TrainingData[i])
        ...
        for j in range(startModelID, startModelID + 10):
            ...
            filename = "./NumericalData/ecoli/Data/Layers_Output/" + listdir_DNNmodel[j][0:-3] + ".csv"
            ...
            for k in range(0,num_samples):
                layer_outputs = get_out_layers(DNNModel,trainingData[k,0:-1],hidden_nodes, num_output)
